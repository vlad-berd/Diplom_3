...
class MainPage(BasePage):
    ...
    def click_on_order_feed_button(self):
        self.click_on_element_action_chains(MainPageLocators.BUTTON_ORDER_FEED, delay=0.1)
    ...
    def close_modal_window_ingredient_details(self):
        ...
        return self.wait_for_element_become_invisible(MainPageLocators.TITLE_FOR_CLOSE_MODAL_WINDOW_INGREDIENT)
    ...
    def get_number_order_id(self, number_order):
        expected_order_number = str(number_order + 1)
        if self.wait_for_text_to_change(MainPageLocators.NUMBER_ORDER_ID, expected_order_number):
            return expected_order_number
    
    @allure.step("Закрыть всплывающее окно оформленного заказа")
    def close_modal_window_order_id(self):
        self.click_on_element_action_chains(MainPageLocators.BUTTON_CLOSE_MODAL_WINDOW_ORDER_ID, delay=0.1)
